...
def poke_list(tkname, L):
    insidestr = " ".join([quote(s) for s in L])
    outsidestr = "[list {}]".format(insidestr)
    tclexec('set '+subst(tkname)+' '+outsidestr)
...
def tree_clear():
    tclexec('$w delete [$w children ""]')

def tree_add(with_id, addl_cols=None, under=None):
    """TODO: TOO COMPLEX

    This should be a series of calls -- the first of which adds a node:
    
      tree_add(nodeid=None, underid=None)
    nodeid is optional, and underid is optional.
    It also cues to the item just added.
    
    tree_find(nodeid) would cue to a node.
    It returns True if it was found, and False if it wasn't.
    
    Then text_set on the tree would work on the cue'd node.
    A particular-case generic function: text_set_kv(k, v),
    would be used to set values underneath a specific column.
    And text_get_kv(k) would return the item under that column.
    This is assuming ttk::treeview will support these operations...
    ...which it may not, just meaning that some gymnastics will have to take place.
    
    tree_find_selected() would return True or False, depending on whether
    tree_find(tree_selected()[0]) was legitimate (tree_selected() returned
    a list) and the first item was found.
    """
    poke("tmp", under if under else "")
    poke("tmp2", with_id)
    if addl_cols:
        poke_list("tmp3", addl_cols)
        tclexec("$w insert $tmp end -id $tmp2 -text $tmp2 -values $tmp3")
    else:
        tclexec("$w insert $tmp end -id $tmp2 -text $tmp2")

def tree_select(L):  # select the items in the list
    """select the items identified in a list
    
    note that this is different than focusing an item (tree_focus)
    """
    poke_list("tmp", L)
    tclexec("$w selection set $tmp")

def tree_selected():
    """return list of selected items"""
    return tclexec("$w selection").split()

def tree_focus(s):
    """focus a specific item"""
    poke("tmp", s)
    return tclexec("$w focus $tmp")
...
def is_focused():
    return bool(int(tclexec("$w instate focus")))
